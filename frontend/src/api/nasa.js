...
export const fetchNasaAsset = async (nasa_id) => {
  const res = await fetch(`${BASE_URL}/api/nasa/asset/${nasa_id}`);
  return res.json();
};

export const fetchNasaMetadata = async (nasa_id) => {
  const res = await fetch(`${BASE_URL}/api/nasa/metadata/${nasa_id}`);
  return res.json();
};

export const fetchNasaCaptions = async (nasa_id) => {
  const res = await fetch(`${BASE_URL}/api/nasa/captions/${nasa_id}`);
  return res.json();
};
...
export const fetchMarsRoverPhotos = async ({ sol, page, camera }) => {
  ...
  const res = await fetch(`${BASE_URL}/api/nasa/mars-rover?${query}`);
  const json = await res.json();
...
export const fetchNEOData = async ({ start_date, end_date }) => {
  const res = await fetch(
    `${BASE_URL}/api/nasa/neo?start_date=${start_date}&end_date=${end_date}`
  );
  const json = await res.json();
